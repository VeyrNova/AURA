...
(()=>{
  ...
  pop.innerHTML=`<header><div><small>AURA WORKSPACE</small><b>MODULES</b></div><button type="button" data-pop-close aria-label="Fermer">×</button></header>
    <div class="aura-p0702-module-grid">
      ...
      <button data-module="documents"><span>${svg('file')}</span><div><b>DOCUMENTS</b><small>Joindre et analyser un fichier</small></div></button>
      <button data-module="system"><span>${svg('system')}</span><div><b>SYSTÈME LIVE</b><small>Ressources et actions locales</small></div></button>
    </div>`;
  ...
  function openPlan(mode){
    ...
    manager()?.open?.('plan');
    setTimeout(()=>{
      const sel=planSubmode==='agenda'?'.aura-p0623-drawer [data-tab="reminders"]':'.aura-p0623-drawer [data-tab="tasks"]';
      q(sel)?.click();setActive(planSubmode);
    },35);
  }
  ...
  function openDocuments(){
    ...
    const attach=q('.workspace>.composer .attach, #composer .attach');
    if(attach&&!attach.classList.contains('disabled')){attach.click();return}
    manager()?.open?.('talk');
  }
  ...
  document.addEventListener('click',event=>{
    ...
    if(tab){if(tab.dataset.tab==='tasks')planSubmode='tasks';if(tab.dataset.tab==='reminders')planSubmode='agenda';setTimeout(syncActive,0)}
    ...
    if(popOpen(settings)&&!event.target.closest('.aura-p0702-settings-popover')&&!event.target.closest('[data-rail-target="settings"]'))closePopover(settings,'settings');
  },true);
