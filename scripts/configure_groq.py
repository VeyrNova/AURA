...
def _clean_key(value: str) -> str:
    ...
    if "=" in key and key.upper().startswith("GROQ_API_KEY="):
...
def _gui_key() -> str:
    ...
    result = {"key": ""}
    root = tk.Tk()
    root.title("AURA — Configuration Groq")
    ...
    info = tk.Label(
        root,
        text=(
            "Colle ta clé API Groq dans le champ ci-dessous.\n"
    ...
    def use_clipboard() -> None:
        ...
        if not valid_cloud_api_token(key):
            messagebox.showerror(
                "Presse-papiers",
                "Aucune clé Groq valide n'a été trouvée dans le presse-papiers.\n\n"
                "Copie d'abord la clé depuis le site Groq puis réessaie.",
                parent=root,
            )
            return
        result["key"] = key
...
def _console_clipboard_flow() -> str:
    ...
    print("1. Copie ta GROQ_API_KEY dans le presse-papiers Windows.")
...
def main() -> int:
    print("============================================================")
    print(" AURA — CONFIGURATION GROQ 26.8.2")
    print("============================================================")
    ...
    if not valid_cloud_api_token(key):
        print()
        print("[ERREUR] Aucune clé Groq valide n'a été fournie.")
        print("Conseil : copie la clé complète, généralement préfixée par 'gsk_', puis relance ce script.")
        return 2

    path = persist_local_env_value("AURA_RUNTIME_MODE", "hybrid")
    persist_local_env_value("GROQ_ENABLED", "true", env_path=path)
    persist_local_env_value("GROQ_STT_ENABLED", "true", env_path=path)
    persist_local_env_value("GROQ_API_KEY", key, env_path=path)

    print()
    print(f"[OK] Configuration enregistrée dans : {path}")
    print("[OK] AURA_RUNTIME_MODE=hybrid")
    print("[OK] GROQ_ENABLED=true")
    print("[OK] GROQ_STT_ENABLED=true")
    print("[OK] GROQ_API_KEY enregistrée (valeur masquée)")
    print()
    print("Redémarre complètement AURA.")
    print("Au prochain démarrage, le journal doit indiquer groq=True si la clé est chargée.")
    return 0
