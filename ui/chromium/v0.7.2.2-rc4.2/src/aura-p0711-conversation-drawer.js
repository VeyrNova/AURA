/* AURA P0.7.1.1 — DRAWER PERSISTENCE + COMPOSER DEDUP
...
(()=>{
  ...
  function syncComposerDedup(){
    const id=currentWorkspace();
    const contextual=isOpen()&&id!=='home'&&id!=='talk';
    document.body.classList.toggle('aura-p0711-drawer-contextual',contextual);
  }
  ...
  function openDirect(){
    ...
    updateContext();syncInputState();renderHistory();clearUnread();setRailState();syncComposerDedup();
  ...
  function closeDirect(){
    ...
    setRailState();syncComposerDedup();
  ...
  window.addEventListener('aura:workspace-will-change',event=>{
    const to=String(event.detail?.to||'').trim().toLowerCase();
    if(isOpen()&&to&&to!=='home'&&to!=='talk')drawerWanted=true;
  });
  window.addEventListener('aura:workspace-changed',event=>{
    const next=String(event.detail?.workspace||currentWorkspace()).trim().toLowerCase();
    updateContext();setRailState();syncComposerDedup();
    // HOME and explicit full Conversation own the surface. Contextual module-to-module
    // switches preserve the drawer, even if a third-party UI transition hid it.
    if(next==='talk'||next==='home'){
      if(isOpen())closeDrawer();else{drawerWanted=false;syncComposerDedup()}
      return;
    }
    if(drawerWanted&&!isOpen()){
      setTimeout(()=>{
        const now=currentWorkspace();
        if(drawerWanted&&!isOpen()&&now!=='home'&&now!=='talk')openDrawer('workspace-persist-recover');
      },35);
    }
  });
  ...
  window.AuraConversationDrawer=Object.freeze({
    ...
    refresh:renderHistory,context:currentWorkspace,wanted:()=>drawerWanted,
  });
