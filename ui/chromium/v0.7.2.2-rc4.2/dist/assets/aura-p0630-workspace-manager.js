...
(()=>{
  ...
  const LABELS={home:'HOME',talk:'TALK',plan:'PLAN',weather:'WEATHER',memory:'MEM',system:'SYS',maps:'MAPS','aura-live':'AURA LIVE'};
  ...
  // AURA LIVE — central ownership of the existing SYSTEM LIVE telemetry node.
  // The node is moved, never cloned, so Project Active and all listeners/state follow it.
  const auraLiveState={
    telemetry:null,
    originParent:null,
  ...
  function auraLiveTelemetry(){
    const cached=auraLiveState.telemetry;
    if(cached?.isConnected)return cached;
    const node=document.querySelector('aside.telemetry.glass');
    if(node)auraLiveState.telemetry=node;
    return node||null;
  }

  function auraLiveCentralHost(){
    if(!workspaceRoot)return null;
    return [...workspaceRoot.children].find(node=>node.matches?.('.hero'))
      || workspaceRoot.querySelector('.hero')
      || null;
  }

  function rememberAuraLiveOrigin(node){
    if(auraLiveState.originParent)return;
    auraLiveState.originParent=node?.parentNode||null;
    auraLiveState.originNextSibling=node?.nextSibling||null;
  }

  function openAuraLive(){
    const node=auraLiveTelemetry();
    const host=auraLiveCentralHost();
    if(!node||!host)return false;
    rememberAuraLiveOrigin(node);
    if(node.parentNode!==host)host.appendChild(node);
    document.body.classList.add('aura-live-active');
    return true;
  }

  function closeAuraLive(){
    const node=auraLiveTelemetry();
    document.body.classList.remove('aura-live-active');
    if(!node)return false;
    const parent=auraLiveState.originParent;
    const next=auraLiveState.originNextSibling;
  ...
  registerWorkspace({
    id:'aura-live',label:'AURA LIVE',
    available:()=>!!auraLiveTelemetry()&&!!auraLiveCentralHost(),
    isOpen:()=>document.body.classList.contains('aura-live-active'),
    open:()=>openAuraLive(),
    close:()=>closeAuraLive(),
    button:()=>document.querySelector('[data-rail-target="aura-live"]'),
    focusTarget:()=>auraLiveTelemetry()?.querySelector('button,[tabindex],input,select,textarea')||auraLiveTelemetry(),
  },{core:true});
