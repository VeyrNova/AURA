...
(()=>{
  ...
  const defs=[
    ...
    ['aura-live','AURA LIVE','diag'],['diagnostics','DIAGNOSTICS','system'],['settings','PARAMÈTRES','settings']
  ];
...
function openPlan(mode){
  ...
  nav.addEventListener('click',event=>{
    ...
    if(id==='aura-live'){manager()?.open?.('aura-live');return}
...
settings.addEventListener('click',e=>{if(e.target.closest('[data-pop-close]'))closePopover(settings,'settings')});
  ...
  function syncActive(){
    ...
    else if(current==='aura-live')setActive('aura-live');
